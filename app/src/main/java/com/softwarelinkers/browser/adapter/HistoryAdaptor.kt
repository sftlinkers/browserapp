...
class HistoryAdapter(private val context: Context, private val isActivity: Boolean = false): RecyclerView.Adapter<HistoryAdapter.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: HistoryAdapter.MyHolder, position: Int) {
        try {
            val icon = BitmapFactory.decodeByteArray(
                MainActivity.historyList[position].image, 0,
                MainActivity.historyList[position].image!!.size)
            holder.image.background = icon.toDrawable(context.resources)
        }catch (e: Exception){
            holder.image.setBackgroundColor(colors[(colors.indices).random()])
            holder.image.text = MainActivity.historyList[position].name[0].toString()
        }
        holder.name.text = MainActivity.historyList[position].name

        holder.root.setOnClickListener{
            when{
                checkForInternet(context) -> {
                    changeTab(
                        MainActivity.historyList[position].name,
                        BrowseFragment(urlNew = MainActivity.historyList[position].url)
                    )
